...
/**
 * Returns new lines with rows and columns swapped.
 * Missing cells in shorter rows are emitted as empty cells.
 *
 * @param {string[]} lines
 * @param {string} delimiter
 */
function transposeLines(lines, delimiter) {
  const rows = lines.map((line) => splitCells(line, delimiter));
  const columnCount = rows.reduce((maximum, row) => Math.max(maximum, row.length), 0);
  const transposed = [];

  for (let column = 0; column < columnCount; column += 1) {
    transposed.push(rows.map((row) => row[column] || '').join(delimiter));
  }
...
function splitCells(line, delimiter) {
  if (delimiter === ' ') {
    return parseSpaceCells(line).map((cell) => cell.content.trim());
  }
...
/**
 * Guesses the delimiter by counting common delimiter occurrences.
 *
 * @param {string[]} lines
 */
function detectDelimiter(lines) {
  ...
  for (const delimiter of candidates) {
    const count = lines.reduce((total, line) => total + countOccurrences(line, delimiter), 0);
