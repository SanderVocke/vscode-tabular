...
function computePostDelimiterSpacing(lines, delimiter) {
  ...
  return parseRows(lines, delimiter).flatMap((row) => row.columns.map((entry) => ({
    lineNumber: row.lineNumber,
 ...
 * @param {string} line
 * @param {string} delimiter
 * @returns {{ content: string, delimiterCharacter: number, delimiterLength: number, existingPadding: number }[]}
 */
function parseColumns(line, delimiter) {
  ...
  for (let column = 0; column < cells.length - 1; column += 1) {
    ...
    columns.push({
      content: cell,
      delimiterCharacter: character + cell.length,
